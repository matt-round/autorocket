...
public:
    ...
    float reward();
    std::tuple<float, float, float, float, float> observationSpaceHigh();
    std::tuple<float, float, float, float, float> observationSpaceLow();
...
private:
    Simulation& m_sim;
    const std::array<float, 5> m_discreteOSWinSize;
    float m_discreteOSSize;
};
